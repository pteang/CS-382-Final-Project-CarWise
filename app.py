...
st.markdown(
    ...
    <div class="scope-note">
      ...
      against the exact imported trim.
    </div>
    """,
    unsafe_allow_html=True,
)
...
submitted = st.button("Search and recommend", type="primary", use_container_width=True)
if submitted:
    if not query.strip():
        st.warning("Enter a vehicle question before searching.")
        st.stop()
        ...
        try:
            ...
            if result.grounded:
                search_status.update(
                    label="Search complete — grounded recommendation ready",
                    state="complete",
                    expanded=True,
                )
            else:
                search_status.update(
                    label="Search complete — no unsupported recommendation produced",
                    state="complete",
                    expanded=True,
                )
    ...
    st.info("Enter a question or choose an example to begin.")
